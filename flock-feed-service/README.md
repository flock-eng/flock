...
Don't forget to add `.github/workflows/flock-feed-service.yaml` to push to docker hub. There is a file called `github-actions-template.yaml` that you can copy and modify. Don't forget to rename it.
...
```bash
docker build -t wcygan/flock-feed-service:latest .
docker push wcygan/flock-feed-service:latest
```

The action is setup to build for `linux/amd64` and `linux/arm64`.

The images are pushed to https://hub.docker.com/r/wcygan/flock-feed-service/tags
...
```bash
kubectl rollout restart deployment/flock-feed-service
```
