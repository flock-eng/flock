import type { Post, PostKey, CreatePostRequest, GetPostRequest, BatchGetPostsRequest, ListMostRecentPostsRequest, ListMostRecentPostsByUserRequest } from '@buf/wcygan_flock.bufbuild_es/backend/v1/post_pb';
import type { MiniProfile, Profile, ProfileKey } from '@buf/wcygan_flock.bufbuild_es/backend/v1/profile_pb';
...
export const createMockApiClient = (): ApiClient => ({
    posts: {
        ...
        batchGetPosts: async (request) => ({
            ...
            posts: (request.ids ?? []).map((id) => createMockPost(id?.id ?? '1', '1', 'Mock post content')),
        }),
        ...
        listMostRecentPostsByUser: async (request) => ({
            ...
            posts: Array.from({ length: request.postLimit ?? 10 }, (_, i) =>
                createMockPost(String(i + 1), request.author?.id ?? '1', `Mock post ${i + 1} by user ${request.author?.id ?? '1'}`),
            ),
        }),
    },
    homePage: {
        getHomePage: async () => ({
            ...
            posts: Array.from({ length: 5 }, (_, i) =>
                createMockPost(String(i + 1), String(i + 1), `Mock home page post ${i + 1}`),
            ),
        }),
    },
    profilePage: {
